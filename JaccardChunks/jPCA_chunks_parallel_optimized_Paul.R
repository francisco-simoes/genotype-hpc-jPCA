...
#for (i in 1:nrow(SM)) #Cycle over individuals)
{
	...
	for(j in 1:nrow(SM))
	{
		different_positive_count[i,j]= (j>=i) * ( sum(GT[i,] == 0 & GT[j,] == 1) +
		sum(GT[i,] == 0 & GT[j,] == 2) +
		sum(GT[i,] == 1 & GT[j,] == 0) +
		sum(GT[i,] == 1 & GT[j,] == 2) +
		sum(GT[i,] == 2 & GT[j,] == 0) +
		sum(GT[i,] == 2 & GT[j,] == 1) ) + (j<i) * different_positive_count[j,i]
		equal_positive_count[i,j]= (j>=i) * ( sum(GT[i,] == 1 & GT[j,] == 1) +
		sum(GT[i,] == 2 & GT[j,] == 2) ) + (j<i) * equal_positive_count[j,i]
	}
}
print('end loop')
time2 <- Sys.time()
